...
function [epis,finalvox_dim,validvol,meanvol,mparams] = preprocessfmri_SA2(figuredir, ...
  fmaps,fmapbrains,fmapsizes,fmapdeltate,fmapunwrap,fmapsmoothing, ...
  epis,mux,vox_dim,inPlaneMatrixSize,TR,sliceOrder,phaseDir,readOutTime,epifmapasst, ...
...
% function [epis,finalvox_dim,validvol,meanvol] = preprocessfmri(figuredir,inplanes,inplanesizes, ...
%   fmaps,fmapbrains,fmapsizes,fmapdeltate,fmapsmoothing, ...
%   epis,vox_dim,inPlaneMatrixSize,TR,sliceOrder,phaseDir,readOutTime,epifmapasst, ...
%   numepiignore,motionreference,motioncutoff,extratrans,targetres,sliceshiftband, ...
%   fmriqualityparams,fmaptimeinterp,mcmask,maskoutnans,epiignoremcvol,dformat)
%
% <figuredir> is the directory to write figures and results to.
%   [] means do not write figures and results.
% <fmaps> is a 3D volume of phase values (in [-pi,pi]) or a cell vector
...
%   volumes supplied.  <fmaps> can be {}, which indicates that no 
%   undistortion is to be applied.
% <fmapbrains> is a 3D volume of magnitude brains or a cell vector
%   of such volumes.  can be {}.  should mirror <fmaps>.
% <fmapsizes> is a 3-element vector with the voxel size in mm
%   or a cell vector of such vectors.  can be {}.  should mirror <fmaps>.
%   if a single vector, we automatically repeat that vector for multiple
%   <fmaps>.
% <fmapdeltate> is the difference in TE that was used for the fmap
%   or a vector of such differences.  should be in milliseconds.  can be [].
%   should mirror <fmaps>.  if a single value, we automatically repeat that 
%   value for multiple <fmaps>.
% <fmapunwrap> is whether to attempt to unwrap the fmap.  can be:
%   (1) 0 means no.
%   (2) 1 means yes and use the '-s -t 0' flags in prelude.
%   (3) a string with the specific flags to use (e.g. '', '-f').
%   can be a cell vector of things like (1)-(3), and can be {}.  should mirror <fmaps>.  
%   if a single element, we automatically repeat that element for multiple <fmaps>.
% <fmapsmoothing> is a 3-element vector with the size of the
%   bandwidth in mm to use in the local linear regression or a cell vector
%   of such vectors.  can be {}.  should mirror <fmaps>.  if a single vector, we 
%   automatically repeat that vector for multiple <fmaps>.  the bandwidth 
%   along each dimension must be greater than the voxel size of the fmap 
...
%   see localregression3d.m, but here is an example: if <fmapsmoothing> is
...
% <mux> # of multiplexed slices acquired simultaneously
% <vox_dim> is a 3-element vector with the voxel size in mm.
% <inPlaneMatrixSize> is [A B] where A and B are the in-plane frequency-encode
...
%   of the EPI data.
% <TR> is the TR in seconds or a vector of TRs.  should mirror <epis>.
%   if a single TR, we automatically repeat that integer for multiple <epis>.
% <sliceOrder> is a vector of positive integers (some permutation
...
%   <sliceOrder> to [] which means do not perform slice time correction.
% <phaseDir> is an integer indicating the phase-encode direction or
%   a vector of such integers.  should mirror <epis>.  if a single integer,
%   we automatically repeat that integer for multiple <epis>.
%   this input matters only if <fmaps> and/or <sliceshiftband> are provided.
...
% <readOutTime> is the duration of the EPI readout in milliseconds.
%   matters only if fmap(s) are provided.
% <epifmapasst> indicates which fmaps to apply to which EPI runs.  
%   matters only if fmap(s) are provided.  if NaN, that means do not 
%   apply any fmaps to any run.  should be a cell vector of the 
...
%   it is a positive integer, that indicates the index of the fmap to use; 
%   if it is 0, that indicates do not apply a fmap for that EPI run; if
%   it is a two-element vector, that indicates to interpolate from time value
%   G to time value H in order to figure out the fmap values to use
%   for that EPI run (in this case, each volume within the run gets a different
%   fmap correction).  if you pass in a vector of non-negative integers,
%   we automatically convert that to a cell vector.  there are some special
%   cases for your convenience.  if <epifmapasst> is passed in as [], 
%   that means (1) if there is one fmap then apply that fmap to all 
%   EPI runs, (2) if there are as many fmaps as EPI runs then apply fmaps
%   1-to-1 to the EPI runs, (3) if there is one more fmap than EPI runs, then
%   interpolate between each successive fmap to obtain the fmap to use
...
% <fmaptimeinterp> (optional) is 'linear' or 'cubic' indicating the type of
%   interpolation to use for the [G H] cases in <epifmapasst>.  note that
...
%   the range of the original fmap time values).  default: 'cubic'.
...
% the apparent motion, (4) undistorting the volumes based on the fmaps (after 
% unwrapping and smoothing the fmaps), (5) estimating motion parameters
...
%  3. for each EPI run, we perform slice time correction according to <sliceOrder>,
...
%  6. if fmaps are provided, we write out the fmaps, fmap brains, and a histogram
%     of fmap values as figures for inspection.  we also write out versions of fmap
%     brains that are matched to the field-of-view and resolution of the EPI data.  we also
%     write out successive differences of the fmaps (e.g. 2-1, 3-2, 4-3, etc.), accounting
%     for phase wraparound.  the range of the fmap figures is -N Hz to N Hz, where N is 
%     1/(<fmapdeltate>/1000)/2.  the range of the fmap difference figures is -50 Hz 
%     to 50 Hz.  the fmap brain figures are individually contrast-normalized.
%  7. we use FSL's prelude utility to unwrap each fmap.
%  8. we write out the unwrapped fmaps as figures for inspection (range same as before).
%     we also write out figures that indicate the weighted mean of the unwrapped fmaps
%     in each slice.  the weights are given by the fmap brains.
%  9. we use local linear regression to smooth the unwrapped fmaps.  we obtain values of
%     the smoothed, unwrapped fmaps only at the locations of the EPI voxels.
% 10. we write out the smoothed, unwrapped fmaps as figures for inspection (range same as before).
%     we also write out versions that are matched to the field-of-view and the resolution
%     of the original fmap data.  we also write out "ALT" versions of these that have
...
%     some of the big inputs to this function (namely, <inplanes>, <fmaps>, <fmapbrains>)
%     and the variables <fmapunwraps>, <sliceshifts>, and <finalfmaps>.
...
%     <finalvox_dim> as a 3-element vector indicating the voxel size of the <epis> output.
%     <validvol> as a logical volume indicating which voxels had no NaNs in any EPI run.
...
% - the undistortion process might assign bright voxel values to regions where fmap measurements are 
...
fmapdiffrng = [-50 50];  % range for fmap difference volumes
...
if ~exist('fmaptimeinterp','var') || isempty(fmaptimeinterp)
  fmaptimeinterp = 'cubic';
end
...
if ~iscell(fmaps)
  fmaps = {fmaps};  % now, it is either {vol}, {vol1 vol2 vol3...}, {A B}, or {}
end
if ~iscell(fmapbrains)
  fmapbrains = {fmapbrains};
end
if ~iscell(fmapsizes)
  fmapsizes = {fmapsizes};
end
if ~iscell(fmapsmoothing)
  fmapsmoothing = {fmapsmoothing};
end
if ~isempty(fmaps)  % disregard the {} case...
  if ~iscell(fmaps{1})
    fmaps = {fmaps};  % now, everything is {A B}, but B could be omitted or []
  end
  if length(fmaps) < 2
    fmaps{2} = [];  % insert B if necessary
  end
  if isempty(fmaps{2})
    fmaps{2} = 1:length(fmaps{1});  % handle [] case for B
  end
  fmaptimes = fmaps{2};  % split off B part into <fmaptimes>
  fmaps = fmaps{1};      % <fmaps> is now just the A part
  % ok, now, it is the case that either:
  % (1) fmaps is {} (which is the no undistortion case) and fmaptimes is not defined
  % (2) fmaps is A (cell vector of fmaps) and fmaptimes is fully specified
end
if ~iscell(epis)
  episissingle = 1;
  epis = {epis};
...
% calc
wantundistort = ~isempty(fmaps);
wantsliceshift = ~isempty(sliceshiftband);

% automatically repeat
if length(fmapsizes)==1
  fmapsizes = repmat(fmapsizes,[1 length(fmaps)]);
end
if length(fmapdeltate)==1
  fmapdeltate = repmat(fmapdeltate,[1 length(fmaps)]);
end
if length(fmapunwrap)==1
  fmapunwrap = repmat({fmapunwrap},[1 length(fmaps)]);
end
if length(fmapsmoothing)==1
  fmapsmoothing = repmat(fmapsmoothing,[1 length(fmaps)]);
end
if length(TR)==1
  TR = repmat(TR,[1 length(epis)]);
end
if (wantundistort || wantsliceshift) && length(phaseDir)==1
  phaseDir = repmat(phaseDir,[1 length(epis)]);
end
...
if isempty(inPlaneMatrixSize)
  inPlaneMatrixSize = sizefull(epis{1},2);
end
...
epifov = epidim .* vox_dim;       % e.g. [128 128 40]

% convert fmapunwrap
for p=1:length(fmapunwrap)
  switch fmapunwrap{p}
  case 1
    fmapunwrap{p} = '-s -t 0';
  end
...
% deal with massaging epifmapasst [after this, epifmapasst will either be NaN or a fully specified cell vector]
if wantundistort
  if isempty(epifmapasst)
    if length(fmaps)==1  % if one fmap, use it for all
      epifmapasst = ones(1,length(epis));
    elseif length(fmaps)==length(epis)  % if equal number, assign 1-to-1
      epifmapasst = 1:length(fmaps);
    elseif length(fmaps)==length(epis)+1  % if one more fmap, then interpolate between successive
      epifmapasst = splitmatrix(flatten([1:length(fmaps)-1; 2:length(fmaps)]),2,2*ones(1,length(epis)));
    else
      error('<epifmapasst> cannot be [] when the number of fmaps is not one NOR the same as the number of EPI runs NOR the same as the number of EPI runs plus one');
    end
  end
  if ~iscell(epifmapasst) && ~isequalwithequalnans(epifmapasst,NaN)
    epifmapasst = num2cell(epifmapasst);
  end
  assert(isequalwithequalnans(epifmapasst,NaN) || (length(epifmapasst)==length(epis)));
end
...
epis = cellfun(@(x,y) x(:,:,:,y(1)+1:end-y(2)),epis,numepiignore,'UniformOutput',0);
  ...
  if ~isempty(sliceOrder)
      ...
      for i = 1:numel(epis)
          ...
          epis{i} = correctSliceTime(epis{i},sliceOrder,mux);
      end
      fprintf('done.\n');
...
% calc fmap stuff
fmapsc = 1./(fmapdeltate/1000)/2;  % vector of values like 250 (meaning +/- 250 Hz)

% write out fmap inspections
if wantfigs && wantundistort
  fprintf('writing out various fmap inspections...');

  % write out fmaps, fmaps brains, and histogram of fmap
  for p=1:length(fmaps)
  
    % write out fmap
    imwrite(uint8(255*makeimagestack(fmaps{p}/pi*fmapsc(p),[-1 1]*fmapsc(p))),jet(256),sprintf('%s/fmap%02d.png',figuredir,p));

    % write out fmap diff
    if p ~= length(fmaps)
      imwrite(uint8(255*makeimagestack(circulardiff(fmaps{p+1},fmaps{p},2*pi)/pi*fmapsc(p), ...
        fmapdiffrng)),jet(256),sprintf('%s/fmapdiff%02d.png',figuredir,p));
    end
  
    % write out fmap brain
    imwrite(uint8(255*makeimagestack(fmapbrains{p},1)),gray(256),sprintf('%s/fmapbrain%02d.png',figuredir,p));
    
    % write out fmap brain cropped to EPI FOV
    imwrite(uint8(255*makeimagestack(processmulti(@imresizedifferentfov,fmapbrains{p},fmapsizes{p}(1:2), ...
      epidim(1:2),vox_dim(1:2)),1)),gray(256),sprintf('%s/fmapbraincropped%02d.png',figuredir,p));

    % write out fmap histogram
    figureprep; hold on;
    vals = prctile(fmaps{p}(:)/pi*fmapsc(p),[25 75]);
    hist(fmaps{p}(:)/pi*fmapsc(p),100);
    straightline(vals,'v','r-');
    xlabel('Fieldmap value (Hz)'); ylabel('Frequency');
    title(sprintf('Histogram of fmap %d; 25th and 75th percentile are %.1f Hz and %.1f Hz',p,vals(1),vals(2)));
    figurewrite('fmaphistogram%02d',p,[],figuredir);
...
fmapunwraps = {};
if wantundistort
  fprintf('unwrapping fmaps if requested...');
  parfor p=1:length(fmaps)
  
    if ~isequal(fmapunwrap{p},0)
  
      % get temporary filenames
      tmp1 = tempname; tmp2 = tempname;
      
      % make a complex fmap and save to tmp1
      save_untouch_nii(make_ana(fmapbrains{p} .* exp(j*fmaps{p}),fmapsizes{p},[],32),tmp1);
      
      % use prelude to unwrap, saving to tmp2
      unix_wrapper(sprintf('prelude -c %s -o %s %s; gunzip %s.nii.gz',tmp1,tmp2,fmapunwrap{p},tmp2));
      
      % load in the unwrapped fmap
      temp = load_nii(sprintf('%s.nii',tmp2));  % OLD: temp = readFileNifti(tmp2);
      
      % convert from radians centered on 0 to actual Hz
      fmapunwraps{p} = double(temp.img)/pi*fmapsc(p);
    
    else
  
      % convert from [-pi,pi] to actual Hz
      fmapunwraps{p} = fmaps{p}/pi*fmapsc(p);
...
% write out inspections of the unwrapping and additional fmap inspections
if wantfigs && wantundistort
  fprintf('writing out inspections of the unwrapping and additional inspections...');
  
  % write inspections of unwraps
  for p=1:length(fmaps)
    imwrite(uint8(255*makeimagestack(fmapunwraps{p},[-1 1]*fmapsc(p))),jet(256),sprintf('%s/fmapunwrapped%02d.png',figuredir,p));
  end

  % write slice-mean inspections
    % this is fmaps x slice-mean with the (weighted) mean of each slice in the fmaps:
  fmapdcs = catcell(1,cellfun(@(x,y) sum(squish(x.*abs(y),2),1) ./ sum(squish(abs(y),2),1),fmapunwraps,fmapbrains,'UniformOutput',0));
  figureprep; hold all;
  set(gca,'ColorOrder',jet(length(fmaps)));
  h = plot(fmapdcs');
  legend(h,mat2cellstr(1:length(fmaps)),'Location','NorthEastOutside');
  xlabel('Slice number'); ylabel('Weighted mean fmap value (Hz)');
  title('Inspection of fmap slice means');
  figurewrite('fmapslicemean',[],[],figuredir);
...
% use local linear regression to smooth the fmaps
smoothfmaps = cell(1,length(fmapunwraps));
if wantundistort && ~isequalwithequalnans(epifmapasst,NaN)
  fprintf('smooth the fmaps...');
  for p=1:length(fmapunwraps)
    if isnan(fmapsmoothing{p})
      smoothfmaps{p} = processmulti(@imresizedifferentfov,fmapunwraps{p},fmapsizes{p}(1:2),epidim(1:2),vox_dim(1:2));
    else
      fsz = sizefull(fmaps{p},3);
      [xx,yy,zz] = ndgrid(1:fsz(1),1:fsz(2),1:fsz(3));
      [xi,yi] = calcpositiondifferentfov(fsz(1:2),fmapsizes{p}(1:2),epidim(1:2),vox_dim(1:2));
      [xxB,yyB,zzB] = ndgrid(yi,xi,1:fsz(3));
      smoothfmaps{p} = nanreplace(localregression3d(xx,yy,zz,fmapunwraps{p},xxB,yyB,zzB,[],[],fmapsmoothing{p} ./ fmapsizes{p},fmapbrains{p},1),0,3);
    end
  end
  fprintf('done.\n');
end
...
% write out smoothed fmap inspections
if wantfigs && wantundistort
  fprintf('writing out smoothed fmaps...');

  % write out fmap and fmap resampled to match the original fmap
  for p=1:length(smoothfmaps)
    if ~isempty(smoothfmaps{p})
      todo = {{1 ''} {1/3 'ALT'}};
      for qqq=1:length(todo)
        imwrite(uint8(255*makeimagestack(smoothfmaps{p},todo{qqq}{1}*[-1 1]*fmapsc(p))),jet(256),sprintf('%s/fmapsmoothed%s%02d.png',figuredir,todo{qqq}{2},p));
        imwrite(uint8(255*makeimagestack(processmulti(@imresizedifferentfov,smoothfmaps{p},vox_dim(1:2), ...
          sizefull(fmaps{p},2),fmapsizes{p}(1:2)),todo{qqq}{1}*[-1 1]*fmapsc(p))),jet(256), ...
          sprintf('%s/fmapsmoothedbacksampled%s%02d.png',figuredir,todo{qqq}{2},p));
      end
    end
  end
...
% deal with epifmapasst
finalfmaps = cell(1,length(epis));  % we need this to exist in all epi cases
if wantundistort
  fprintf('deal with epi fmap assignment and time interpolation...');

  % calculate the final fmaps [we use single to save on memory]
  if ~isequalwithequalnans(epifmapasst,NaN)
    for p=1:length(epifmapasst)
      if epifmapasst{p} ~= 0
        fn = epifmapasst{p};
        
        % if scalar, just use as-is, resulting in X x Y x Z
        if isscalar(fn)
          finalfmaps{p} = single(smoothfmaps{fn});
        
        % if two-element vector, do the interpolation, resulting in X x Y x Z x T     [[OUCH. THIS DOUBLES THE MEMORY USAGE]]
        else
          finalfmaps{p} = single(permute(interp1(fmaptimes,permute(catcell(4,smoothfmaps),[4 1 2 3]), ...
                                                     linspace(fn(1),fn(2),size(epis{p},4)),fmaptimeinterp,'extrap'),[2 3 4 1]));
        end
...
if wantfigs && wantundistort
  ...
  temp = cellfun(@(x,y,z) undistortvolumes(x(:,:,:,[1 end]),vox_dim, ...
                 y(:,:,:,[1 end])*(readOutTime/1000)*(epidim(abs(z))/inPlaneMatrixSize(2)), ...
                 z,[]),epis,finalfmaps,num2cell(phaseDir),'UniformOutput',0);
...
if wantsliceshift
  ...
  sliceshifts = cellfun(@(x,y) single(repmat(x(1,y,:,:),epidim(1:2))),combandpass,num2cell(abs(phaseDir)),'UniformOutput',0);
...
if wantmotioncorrect
  ...
  if wantsliceshift
    [epistemp,d,validvoltemp] = cellfun(@(x,y,z) undistortvolumes(x, ...
                       vox_dim,y,z,[]),epis,sliceshifts,num2cell(abs(phaseDir)),'UniformOutput',0);
  ...
  if wantundistort
    [epistemp,d,validvoltemp] = cellfun(@(x,y,z) undistortvolumes(x,vox_dim, ...
      y*(readOutTime/1000)*(epidim(abs(z))/inPlaneMatrixSize(2)),z,[]),epistemp,finalfmaps,num2cell(phaseDir),'UniformOutput',0);
    % yuck..  we have to explicitly convert to double/single and then set nan voxels to NaN
    for p=1:length(epistemp)
      epistemp{p} = squish(cast(epistemp{p},dformat),3);
      epistemp{p}(find(~validvoltemp{p}),:) = NaN;
  ...
  [epistemp,mparams] = motioncorrectvolumes(epistemp,cellfun(@(x,y) [x y],repmat({vox_dim},[1 length(epis)]),num2cell(TR),'UniformOutput',0), ...
  ...
  if wantundistort
    if wantsliceshift
      [epis,voloffset,validvolrun] = cellfun(@(x,y0,y,z,w) undistortvolumes(x, ...
                     vox_dim,bsxfun(@plus,sign(z)*y0, ...
                                    y*(readOutTime/1000)*(epidim(abs(z))/inPlaneMatrixSize(2))),z,w(2:end,:),extratrans,targetres(1:3)), ...
                     epis,sliceshifts,finalfmaps,num2cell(phaseDir),mparams,'UniformOutput',0);
    else
      [epis,voloffset,validvolrun] = cellfun(@(x,y,z,w) undistortvolumes(x,vox_dim, ...
                     y*(readOutTime/1000)*(epidim(abs(z))/inPlaneMatrixSize(2)),z,w(2:end,:),extratrans,targetres(1:3)), ...
                     epis,finalfmaps,num2cell(phaseDir),mparams,'UniformOutput',0);
    end
  else
    if wantsliceshift
      [epis,voloffset,validvolrun] = cellfun(@(x,y0,z,w) undistortvolumes(x, ...
                     vox_dim,y0,z,w(2:end,:),extratrans,targetres(1:3)), ...
                     epis,sliceshifts,num2cell(abs(phaseDir)),mparams,'UniformOutput',0);
    else
      [epis,voloffset,validvolrun] = cellfun(@(x,w) undistortvolumes(x, ...
                     vox_dim,[],[],w(2:end,:),extratrans,targetres(1:3)), ...
  ...
  if wantundistort
    if wantsliceshift
      [epis,voloffset,validvolrun] = cellfun(@(x,y0,y,z) undistortvolumes(x, ...
                     vox_dim,bsxfun(@plus,sign(z)*y0, ...
                                    y*(readOutTime/1000)*(epidim(abs(z))/inPlaneMatrixSize(2))),z,[],extratrans,targetres(1:3)), ...
                     epis,sliceshifts,finalfmaps,num2cell(phaseDir),'UniformOutput',0);
    else
      [epis,voloffset,validvolrun] = cellfun(@(x,y,z) undistortvolumes(x,vox_dim, ...
                     y*(readOutTime/1000)*(epidim(abs(z))/inPlaneMatrixSize(2)),z,[],extratrans,targetres(1:3)), ...
                     epis,finalfmaps,num2cell(phaseDir),'UniformOutput',0);
    end
  else
    if wantsliceshift
      [epis,voloffset,validvolrun] = cellfun(@(x,y0,z) undistortvolumes(x, ...
                     vox_dim,y0,z,[],extratrans,targetres(1:3)), ...
                     epis,sliceshifts,num2cell(abs(phaseDir)),'UniformOutput',0);
    else
      [epis,voloffset,validvolrun] = cellfun(@(x) undistortvolumes(x, ...
                     vox_dim,[],[],[],extratrans,targetres(1:3)), ...
...
if iscell(targetres)
  finalvox_dim = targetres{2};
else
  finalvox_dim = epifov ./ targetres;                                        % size in mm of a voxel in the final EPI version
end
...
clear fmaps fmapbrains fmapunwraps;
clear sliceshifts finalfmaps;
  
% do fMRI quality
if wantfigs && ~iscell(targetres) && ~isequalwithequalnans(fmriqualityparams,NaN)
  fprintf('calling fmriquality.m on the epis...');
  fmriquality(epis,vox_dim,[figuredir '/fmriquality'],fmriqualityparams{:});  % note that NaNs are present and may be in weird places...
...
% internal constants
%fmapseedfctr = 5;    % number of seeds along each dimension of the fmap
%fmapptile = 99;      % percentile of absolute value of fmap values for scaling purposes
%fmapbptile = 99;     % percentile of fmap brain values for scaling purposes
% fmapmaskpct = 99;    % percentile of fmap brain for the mask
% fmapmaskfctr = 0.1;  % multiplier on the percentile for the mask



  %     % try many starting seeds for the unwrapping
  %     ii = resamplingindices(1,size(fmaps{p},1),fmapseedfctr);
  %     jj = resamplingindices(1,size(fmaps{p},2),fmapseedfctr);
  %     kk = resamplingindices(1,size(fmaps{p},3),fmapseedfctr);
  %     fmapedgepower = zeros(fmapseedfctr,fmapseedfctr,fmapseedfctr);
  %     for i1=1:length(ii)
  %       for i2=1:length(jj)
  %         for i3=1:length(kk)
  %           temp = robustunwrap([ii(i1) jj(i2) kk(i3)],fmaps{p}+pi,fmapbrains{p});
  %           fmapedgepower(i1,i2,i3) = sum(flatten(detectedges(temp,1).^2 .* abs(fmapbrains{p})));  % abs for safety
  %         end
  %       end
  %     end
  %     
  %     % which was best
  %     [d,ix] = min(fmapedgepower(:));
  %     [i1,i2,i3] = ind2sub([fmapseedfctr fmapseedfctr fmapseedfctr],ix);
  %     
  %     % do the unwrapping one last time
  %     fmapunwraps{p} = robustunwrap([ii(i1) jj(i2) kk(i3)],fmaps{p}+pi,fmapbrains{p});
  %     
  %     % convert from [0,2*pi] to actual Hz
  %     fmapunwraps{p} = (fmapunwraps{p}-pi)/pi*fmapsc(p);

      % % calculate fmap-related ranges
      % if wantundistort
      %   fmapmx = prctile(abs(catcell(1,cellfun(@(x) x(:),fmaps,'UniformOutput',0))),fmapptile);    % [-A A] range for fmaps
      %   fmapbmx = prctile(catcell(1,cellfun(@(x) x(:),fmapbrains,'UniformOutput',0)),fmapbptile);  % [0 B] range for fmap brains
      % end
