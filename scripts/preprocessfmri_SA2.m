...
%              %% try this instead for mux3
             fprintf('correcting for differences in slice acquisition times for MUX3 sequence...');
             mux3sliceorder = repmat(calcposition(episliceorder,1:max(episliceorder)),1,3);
             epis = cellfun(@(x,y) sincshift(x,repmat(reshape((1-y)/max(y),1,1,[]),[size(x,1) size(x,2)]),4), ...
                 epis,repmat({mux3sliceorder},[1 length(epis)]),'UniformOutput',0);
